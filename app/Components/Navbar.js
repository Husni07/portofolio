    ...
    <div className='flex w-full justify-center items-center h-[4.5rem] fixed shadow-sm px-5 xl:px-0 z-[999]'>
        <div className='flex w-[75rem] justify-between items-center text-slate-800'>
            ...
            <div className="flex gap-10 items-center">
                <ul className={`hidden md:flex gap-8 text-lg font-medium w-full transition-all duration-300 ease-in`}>
                    <li className="ease-linear duration-200 hover:text-white"><a href="#home">Home</a></li>
                ...
                <div className="relative flex flex-col" ref={divRef}>
                    ...
                    <div className={`${open ? "opacity-100 translate-y-7" : "invisible"} shadow-sm rounded-md p-2 w-44 opacity-0 bg-white absolute top-8 right-0 transition-all flex flex-col gap-2 z-99`}>
                        <a href="#home" className="flex md:hidden justify-between text-black p-2 bg-slate-50 rounded-md items-center shadow-inner cursor-pointer transition-all hover:bg-slate-100">
                            <h1 className='font-semibold text-sm'>Home</h1>
                        </a>
                        <a href="#about" className="flex md:hidden justify-between text-black p-2 bg-slate-50 rounded-md items-center shadow-inner cursor-pointer transition-all hover:bg-slate-100">
                            <h1 className='font-semibold text-sm'>About</h1>
                        </a>
                        <a href="#projects" className="flex md:hidden justify-between text-black p-2 bg-slate-50 rounded-md items-center shadow-inner cursor-pointer transition-all hover:bg-slate-100">
